...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_FILES['image_file'])) {
        ...
        if ($data['tmp_path'] && $image['error'] === UPLOAD_ERR_OK) {
            ...
            $type = mime_content_type($data['tmp_path']);
            $errors['image_file'] .= in_array($type, MEDIA_TYPES) ? '' : 'The file type is not allowed';
            ...
            if ($errors['image_file'] === '' && $errors['image_alt'] === '') {
                ...
                $data['save_to'] = get_file_path($image['name'], UPLOAD_DIR);
            }
        }
    }
    ...
    if (!$problems) {
        ...
        if ($data['save_to']) {
            scale_and_copy($data['tmp_path'], $data['save_to']);
