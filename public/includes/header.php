                ...
                <div class="hidden w-full md:block md:w-auto" id="navbar-default">
                    <ul id="menu" class="font-medium flex flex-col p-4 md:p-0 mt-4 border border-gray-100 rounded-lg bg-gray-50 md:flex-row md:space-x-8 rtl:space-x-reverse md:mt-0 md:border-0 md:bg-white dark:bg-gray-800 md:dark:bg-gray-900 dark:border-gray-700">
                        <?php foreach ($navigation as $link) : ?>
                            <li>
                                <a class="block py-2 px-3 text-white bg-blue-700 rounded hover:text-pink-600 md:bg-transparent md:text-blue-700 md:p-0 dark:text-white md:dark:text-blue-500" href="category.php?id=<?= $link['id'] ?>" <?= ($section == $link['id']) ? 'aria-current="page"' : '' ?>>
                                    <?= e($link['name']) ?>
                                </a>
                            </li>
                        <?php endforeach; ?>
